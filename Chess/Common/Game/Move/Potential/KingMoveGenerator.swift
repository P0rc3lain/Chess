    ...
    func potentialActions(piece: Piece, state: GameState) -> [Action] {
        ...
        if !browser.wasEverMoved(piece: piece, state: state) {
            ...
            if let rookAField = interactor.field(of: rookA, board: state.board),
               !browser.wasEverMoved(piece: rookA, state: state) {
                if piece.color == .black {
                    ...
                    if (pieceField.row + 1 ..< rookAField.row).allSatisfy({ index in
                        return state.board.fields[index][rookAField.column] == nil
                    }) {
                        standard += [
                            Action(mainMove: (pieceField, Field(pieceField.row + 2, rookAField.column)),
                                   sideEffects: [Move(who: rookA,
                                                      from: rookAField,
                                                      to: Field(rookAField.row - 3, rookAField.column))],
                                   piecesToAdd: [],
                                   piecesToRemove: [])
                        ]
                    }
            ...
            if let rookBField = interactor.field(of: rookB, board: state.board),
               !browser.wasEverMoved(piece: rookB, state: state) {
                if piece.color == .black {
                    ...
                    if (rookBField.row + 1 ..< pieceField.row).allSatisfy({ index in
                        return state.board.fields[index][rookBField.column] == nil
                    }) {
                        standard += [
                            Action(mainMove: (pieceField, Field(pieceField.row - 2, rookBField.column)),
                                   sideEffects: [Move(who: rookB,
                                                      from: rookBField,
                                                      to: Field(rookBField.row + 2, rookBField.column))],
                                   piecesToAdd: [],
                                   piecesToRemove: [])
                        ]
                    }
